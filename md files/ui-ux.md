...
- That seat's own last Play is shown at full visibility only while it is still the live hand to beat, and is **dimmed ("grayed out")** as soon as it is not — once a later Play beats it, or once the Trick it belonged to has concluded. It is not removed, so the player can see what was beaten and by how much. Full visibility therefore always means "this is the hand currently on the table", never merely "nobody topped it".
...
- **Current Turn:** cyan glow (`#22d3ee`) — a hue none of the gold/silver/bronze finish glows use, so it is never confused with any of them (an earlier white glow read as too close to the silver 2nd-place glow).
...
Both overlays derive from authoritative public Engine history/events. While either overlay is open, Orchestrator progression is paused so bot actions do not occur unseen. Closing resumes from the same execution point. While either overlay (or the Leave confirmation, §10) is open, the table beneath it is also inert: pointer and keyboard input cannot reach Play/Pass, the hand, or the other table controls, and a Play/Pass submission that still reaches the handler while paused is rejected. Closing an overlay returns keyboard focus to the control that opened it. A Round Result, Session Summary, reveal-skip or Round-start screen that ends up behind such an overlay (a Round can end while one is open) is inert too: its controls, including Next Round, receive no input until the overlay closes.
...
Show the reveal for roughly **1.5–2 seconds**. A click/tap may finish the reveal immediately. The same input must not accidentally activate the next result action. For the whole reveal, table controls (Event Log, Discard Pile, Leave Game, the hand, Play/Pass) cannot be reached by pointer or keyboard, and the revealed cards are not dimmed; the table only dims once the Result overlay opens (post-M4-T14.5 review fix).
...
The animation is short and skippable by click/tap; skipping completes the visual state but must not trigger Next Round (or, for Round 5, the automatic Session Summary transition below). Ties during an unfinished Session retain stable previous relative order (the order shown before this Round's points were added, carried forward Round to Round) rather than inventing a final ranking.
