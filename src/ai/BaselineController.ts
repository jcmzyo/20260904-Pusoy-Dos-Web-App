...
export class BaselineController implements PlayerController {
  constructor(readonly playerId: PlayerId, private readonly ruleset: RulesetConfig = defaultRuleset, private readonly onDecision?: (trace: DecisionTrace) => void) {}
  ...
  async chooseMove(request: PlayerTurnRequest): Promise<Move> {
    ...
    return chooseBaselineMove(request, this.ruleset, this.onDecision);
  }
}
